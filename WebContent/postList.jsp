...
<body>
	...
	<button onclick="location.href='postWriteForm.jsp'">레시피작성</button>
		<form action="category" id="frm" method="get">
			<div>
				<input type="radio" id="all" name="categoryId" value="0" onclick="location.href='./postList'"  <c:if test="${all != null || list[0].categoryId == null ||all.equals(all) }">checked="checked"</c:if> />전체
				&nbsp; 
				<input type="radio" id="chinaFood" name="categoryId" value="2" onclick="document.getElementById('frm').submit();" <c:if test="${list[0].categoryId == 2 && category.equals(category) && notAll.equals(notAll)}">checked="checked"</c:if> />한식
				&nbsp;
				<input type="radio" id="koreanFood" name="categoryId" value="3" onclick="document.getElementById('frm').submit();" <c:if test="${list[0].categoryId == 3 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if>/>중식
				&nbsp;
				<input type="radio" id="japaneseFood" name="categoryId" value="4" onclick="document.getElementById('frm').submit();"<c:if test="${list[0].categoryId == 4 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if> />일식
				&nbsp;
				<input type="radio" id="westernFood" name="categoryId" value="5" onclick="document.getElementById('frm').submit();"<c:if test="${list[0].categoryId == 5 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if> />양식
				&nbsp;
				<input type="radio" id="snackFood" name="categoryId" value="6" onclick="document.getElementById('frm').submit();"<c:if test="${list[0].categoryId == 6 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if>/>분식
				&nbsp;
				<input type="radio" id="vegetarian" name="categoryId" value="7" onclick="document.getElementById('frm').submit();"<c:if test="${list[0].categoryId == 7 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if>/>채식
				&nbsp;
				<input type="radio" id="babyFood" name="categoryId" value="8" onclick="document.getElementById('frm').submit();"<c:if test="${list[0].categoryId == 8 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if>/>이유식
				&nbsp;
				<input type="radio" id="dessert" name="categoryId" value="9" onclick="document.getElementById('frm').submit();" <c:if test="${list[0].categoryId == 9 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if>/>디저트
				&nbsp;
				<input type="radio" id="etc" name="categoryId" value="1" onclick="document.getElementById('frm').submit();" <c:if test="${list[0].categoryId == 1 && category.equals(category)&& notAll.equals(notAll)}">checked="checked"</c:if> />기타
			</div>
		</form>
			...
			<form action="postSearch" method="get" id="postSearch">
				<select name="postSearchOpt" id="postSearchOpt">
					...
					<span id="option1"><input type="text" id="title_contentsSearch" name="keyword" placeholder="제목 + 내용 입력" /></span>
					...
					<span hidden="hidden" id="option3"><input type="text" id="nickNameSearch" name="keywordNickName" placeholder="닉네임 입력" /></span>
					<span hidden="hidden" id="option4"><input type="text" id="itemSearch" name="keywordItem" placeholder="재료 입력" /></span>
         			<c:if test="${category.equals(category) && total != 0 && notAll.equals(notAll)}"><input type="hidden" name="categoryId" value="${list[0].categoryId}"/></c:if>
         			<c:if test="${all.equals(all) || total == 0}"><input type="hidden" name="categoryId" value="0"/></c:if>
					<button>검색</button>
				</form>
	...
	<c:forEach var="i" begin="${start }" end="${end}" step="1">
		<span class="postPage">
			...
			<c:if test="${i ne currPage && searchPage.equals(searchPage)}"><a href="./postSearch?postSearchOpt=${postSearchOpt}&keyword=${keyword}&keywordMin=${keywordMin}&keywordMax=${keywordMax}&categoryId=${categoryId}&postPage=${i}">${i}</a></c:if>
