	...
	<div class="wrap">
		...
		<main id="body">
			<a href='./myPage' id='backToMyPage'><h4
					...
					← MY PAGE로 돌아가기</h4></a>
			<h3>${sessionScope.nickName}님의주문 내역</h3>
			...
			<div id='orderDetail' style='background-color: snow;'>
				...
				<div>총 할인액&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-${map.discount}
					원</div>
				<div>총 결제금액 :: ${map.paymentPrice} 원</div>

				<c:forEach items="${map.orderList}" var="item">
					<hr />
					<a href="./shopDetail?productId=${item.productId}"><div
							id='orderItem'>
							<div>
								<img src='/photo/${item.imgNewName}' alt='주문상품' width='120'
									height='100' />
							</div>
							<div>${item.productName}</div>
							<div>수량 &nbsp;${item.productCount} 개
								&nbsp;&nbsp;|&nbsp;&nbsp;${item.price} 원</div>
						</div> </a>
				</c:forEach>
