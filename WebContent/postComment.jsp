...
<script>
				...
				.ajax({
					...
					success : function(data) {
						...
						if (data.list != null) { //DB에서 댓글 데이터를 정상적으로 가져왔다면
							...
							drawComments(data.list, data.totalPage,
									data.currPage, data.start, data.end); //댓글리스트를 브라우저에 그려준다
						} else { //등록된 댓글이 없다면
							$("#emptyComment")
									.html(
	...
	function drawComments(list, totalPage, currPage, start, end) {
		...
		list
				...
				content +=	" id='cmtContentUser' alt=\"댓글작성자이미지\" width=80px height=80px /></td>";
						...
						if (item.userId == loginId) {
							...
							content += "<td style='color:rosybrown;'><b>작성자</b> "
									+ item.nickName + "</td>";
							content += "<td style='color:rosybrown;'><b>등급</b> "
									+ item.rankName + "</td>";
						} else {
							...
							content += "<td><b>작성자</b> " + item.nickName
									+ "</td>";
							content += "<td><b>등급</b> " + item.rankName
									+ "</td>";
						}
						...
						content += "<td><button id='cmtReplyBtn" + idx
								+ "' onclick='cmtReplyClick(" + idx
								+ ")'>답글</button></td>";

						//이 댓글의 작성자가 현재 로그인된 유저아이디와 같지않다면 신고 버튼을 보여준다.
						if (item.userId != loginId) {
							content += "<td><button id='reportBtn" + idx
									+ "' onclick='cmtReportBtnClick(" + idx
									+ ", \"" + item.commentId + "\", \""
									+ item.lev + "\", \"" + item.nickName
									+ "\")'>신고</button></td>";
						} else {
							content += "<td></td>";
						}
						...
						if (loginIdIsAdmin == "Y") {
							content += "<td><button id='blindBtn" + idx
									+ "' onclick='cmtBlindBtnClick(" + idx
									+ ", \"" + item.commentId + "\", \""
									+ item.lev + "\", \"" + item.nickName
									+ "\")'>블라인드</button></td>";
						} else {
				...
				content +=	"style='width:500px;'>"
						...
						content += "</td>";
						content += "<tr>"
						content += "<td></td><td><b style='color:rosybrown;'>&nbsp;♥ 대댓글 작성</b></td><td></td><td style='color:rosybrown;'><b>작성자</b> "
								+ loginNickName + "</td>";
						content += "<td><button id='cmtUpdateSubmitBtn" + idx
	...
	function cmtReplyExecute(idx, commentId) {
		console.log("cmtReplyExecute() idx/commentId : " + idx + "/"
				+ commentId);
		console.log("cmtReplyExecute() 작성내용 : "
				+ $("#cmtReplyInput" + idx).val());

		if ($("#cmtReplyInput" + idx).val() == null
				|| $("#cmtReplyInput" + idx).val() == "") {
	...
	function cmtDelete(idx, commentId, lev) {
		console.log("cmtDelete() idx/commentId/lev : " + idx + "/" + commentId
				+ "/" + lev);
	...
	function cmtReportBtnClick(idx, commentId, lev, nickName) { //idx, (대)댓글번호, 레벨(댓글인지 대댓글인지), 댓쓴이 닉네임
		console.log("cmtReportBtnClick() idx/commentId/lev/nickName : " + idx
				+ "/" + commentId + "/" + lev + "/" + nickName);
		var url = "";
		if (lev == "comment") {
			url = "./memberReport?classification=C&&postId=" + commentId
					+ "&&nickName=" + nickName;
		} else {
			url = "./memberReport?classification=R&&postId=" + commentId
					+ "&&nickName=" + nickName;
		}
		console.log("url : " + url);
	...
	function cmtBlindBtnClick(idx, commentId, lev, nickName) { //idx, (대)댓글번호, 레벨(댓글인지 대댓글인지), 댓쓴이 닉네임
		console.log("cmtBlindBtnClick() idx/commentId/lev : " + idx + "/"
				+ commentId + "/" + lev + "/" + nickName);
		var url = "";
		if (lev == "comment") {
			url = "./memberBlind?classification=C&&postId=" + commentId
					+ "&&nickName=" + nickName;
		} else {
			url = "./memberBlind?classification=R&&postId=" + commentId
					+ "&&nickName=" + nickName;
		}
		var option = "width=650, height=500, top=500, location = no, resizable = no";
		console.log("url : " + url);
